...
import docutils
from doc import tests_html_parser
...
class testDirective(Directive):
    # The class enables content in the directive
    has_content = True
    add_index = True

    option_spec = {
        'resources': docutils.parsers.rst.directives.unchanged,
...
def process_test_nodes(app, doctree, fromdocname):
    ...
    for node in doctree.traverse(testlist):
        ...
        for test_ in (t for t in tests if isinstance(t.resources, str)):
        ...
        for tag in tags:
            ...
            t["tests"] = sorted(t["tests"], key=lambda el: el['description'])
            t["tests"] = sorted(t["tests"], key=lambda el: el['titel'])
            setTests.append(t)

        setTests = sorted(setTests, key=lambda el: el['tag'])
