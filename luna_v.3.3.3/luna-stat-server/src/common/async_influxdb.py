...
from common.api_tools import BadRequest
...
class InfluxDBServerUnavailableError(InfluxDBServerError):
    def __init__(self, e):
        Exception.__init__(self, f'InfluxDB server unavailable due to: {e}')


class AsyncInfluxDBClient(object):
    """The :class:`~.InfluxDBClient` object holds information necessary to
    connect to InfluxDB. Requests can be sent to InfluxDB directly through
    the client.

    :param host: hostname to InfluxDB, defaults to 'localhost'
    :type host: str
    :param port: port to InfluxDB, defaults to 8086
    :type port: int
    :param username: user to connect, defaults to 'root'
    :type username: str
    :param password: user password, defaults to 'root'
    :type password: str
    ...
    :param timeout: number of seconds requests wait for your client to
    ...
    def __init__(
        ...
        if proxies is None:
            ...
            _, self._proxies = proxies.items()[0]
    ...
    def from_DSN(dsn, **kwargs):
        """Return the :class:`~.InfluxDBClient` instanse from the provided
...
localhost:8086/databasename', timeout=5, udp_port=159)
        ...
        return AsyncInfluxDBClient(**init_args)

    def switch_database(self, database):
        """Change the client's database.

        :param database: new client's database name
        :type database: str
        """
        self._database = database
    ...
    async def request(self, url, method='GET', params=None, data=None,
        ...
        :param url: the HTTP-request path, e.g. write, query, etc.
        :type url: str
        ...
        if 'q' in params:
            params['q'] = params['q'] + f' TZ(\'{get_localzone()}\')'
        ...
        # Try sending the request three times maximum. (see #103)
        ...
        try:
            ...
            async with self._session.request(
                ...
                response_content = (await response.read()).decode()
        except (asyncio.TimeoutError, ServerTimeoutError) as e:
            raise InfluxDBServerUnavailableError(e)

        if 500 <= response_status_code < 600:
            raise InfluxDBServerError(response_content)
        elif response_status_code == expected_response_code:
            return json.loads(response_content) if len(response_content) > 0 else {}
        else:
            if response_content.endswith(''' ': invalid field format"}\n'''):
                raise BadRequest(description='"InfluxDB: Invalid values given"')
            raise BadRequest(description=response_content)
    ...
    async def write(self, data, params=None, expected_response_code=204,
        ...
        :type expected_response_code: int
        :param protocol: input data protocol, either 'json' or 'line'
        :type protocol: str
    ...
    async def query(self,
        ...
        :param expected_response_code: the expected response status code,
            defaults to 200
        :type expected_response_code: int
        ...
        :param raise_errors: Whether raise exceptions when InfluxDB
            returns errors or not, defaults to True
    ...
    async def write_points(self,
        ...
        :param tags: a set of key-value pairs associated with each point. Both
            keys and values must be strings. These are shared tags to be
        ...
        :param batch_size: value to write the points in batches
            instead of all at once. Useful when doing data dumps from
    ...
    async def get_list_database(self):
        ...
        """
        return list(await self.query("SHOW DATABASES").get_points())

    async def create_database(self, dbname):
        """Create a new database in InfluxDB.

        :param dbname: new name of the database
        :type dbname: str
        """
        await self.query("CREATE DATABASE \"%s\"" % dbname)
    ...
    async def drop_database(self, dbname):
        ...
        :type dbname: str
        """
        await self.query("DROP DATABASE \"%s\"" % dbname)
    ...
    async def create_retention_policy(self, name, duration, replication,
        ...
        :param duration: the duration of the new retention policy.
            ...
            and mean 1 hour, 90 minutes, 12 hours, 7 days, and 4 weeks,
            respectively. For infinite retention – meaning the data is
            never deleted – use 'INF' for the duration.
        ...
        :type database: str
        :param default: whether set the policy as default or not
        :type default: bool
        """
        query_string = \
    ...
    async def alter_retention_policy(self, name, database=None,
        ...
        :param replication: the new replication of the existing
            retention policy
        :type replication: str
        :param default: whether to set the modified policy as default or not 
        :type default: bool

        .. note:: at least one of duration, replication, or default flag
            should be set. Otherwise, the operation fails.
        """
        query_string = (
    ...
    async def get_list_retention_policies(self, database=None):
        """Get the list of retention policies for the database.
    ...
    async def delete_series(self, database=None, measurement=None, tags=None):
        ...
        :param database: the database from where the series should be
            deleted, defaults to client's current database
        :type database: str
        :param measurement: Delete all series from a measurement
        :type id: str
        :param tags: Delete all series that match the given tags
        :type id: dict
        """
    ...
    async def get_list_privileges(self, username):
        """Get the list of all privileges granted to the user.
    ...
    async def send_packet(self, packet, protocol='json'):
        """Send an UDP packet.
