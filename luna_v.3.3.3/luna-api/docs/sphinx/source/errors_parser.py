...
rows.append('===========')
rows.append('|{:<30}|{:<10}|{:<75}|'.format("Error", "Error code", "Error description"))
rows.append('|{:-^30}|{:-^10}|{:-^75}|'.format("", "", ""))

with codecs.open('../../../luna_api/crutches_on_wheels/errors/errors.py', 'r', 'utf-8') as f:
    for line in f:
        ...
        if ei in tokens:
            ...
            # make table row
            tr = '|{:<30}|{:<10}|{:<75}|'.format(tokens[0], tokens[ec_i], desc)
