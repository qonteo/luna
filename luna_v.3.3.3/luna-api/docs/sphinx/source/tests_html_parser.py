...
def generateRowSetTestsMD(tests, tag):
    ...
    rows = [tag]
    rows.append(('{:-^' + '{}'.format(len(tag)) + '}').format(""))
    ...
    formatSeparatorHeaders = '|{:-^' + '{}'.format(lengthTest) + \
                             ...
                             '}|{:-^' + '{}'.format(lengthDescription) + \
                             '}|{:-^' + '{}'.format(lengthS3) + '}|'
    rows.append(formatRow.format("Test", "Resources", "Test description", "S3"))
    rows.append(formatSeparatorHeaders.format("", "", "", ""))
    for test in tests:
        countResource = len(test.resources)
        if countResource > 0:
            r = formatRow.format(test.titel, test.resources[0], test.description, test.S3)
        else:
            r = formatRow.format(test.titel, "", test.description, test.s3)
        rows.append(r)
...
def generateRowSetTestsReStr(tests, tag):
    ...
    rows.append(formatRow.format("Test", "Resourses", "Test description", "S3"))
    rows.append(formatSeparatorHeaders.format("", "", "", ""))
...
def generateTestsNode(tests):
    def addTestRow(test, tbody):
        ...
        entry = nodes.entry(morerows = rowSpawn)
        row += entry
        entry += nodes.paragraph(text = test.S3)
        tbody += row
        for resource in test.resources[1:]:
            row = nodes.row()
    ...
    headers = ('Test', 'Resourses', 'Description', 'S3')
    table = nodes.table()

    tgroup = nodes.tgroup(cols = len(headers))
    colwidths = (2, 1.75, 4, 0.25)
    table += tgroup
