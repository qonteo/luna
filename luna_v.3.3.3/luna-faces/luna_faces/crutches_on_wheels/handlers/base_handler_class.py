...
class VLBaseHandler(web.RequestHandler):
    ...
    def getPagination(self, defaultPage: int = 1, defaultPageSize: int = 10, maxPageSize: int = 100,
                      minPageSize: int = 1) -> Tuple[int, int]:
        """
        Getting pagination

        Args:
            maxPageSize: max elements by page
            minPageSize: min elements by page
        ...
        pageSize = self.getQueryParam("page_size", lambda params: max(min(int(params), maxPageSize), minPageSize),
