...
def timeFilterGetter(value: str) -> datetime:
    """
    ...
    >>> timeFilterGetter("10-12-2018T12:00:00+00:00")
    datetime.datetime(2018, 12, 10, 12, 0)
    >>> timeFilterGetter("10-12-2018T12:00:00Z")
    datetime.datetime(2018, 12, 10, 12, 0)
    """
    datetimeValue = parser.parse(value.replace(' ', '+'))
    if not value.endswith('Z') and STORAGE_TIME == 'UTC':
        return datetimeValue.astimezone(pytz.UTC).replace(tzinfo=None)
    elif STORAGE_TIME == 'LOCAL':
        return datetimeValue.astimezone(tz.tzlocal())
    else:
